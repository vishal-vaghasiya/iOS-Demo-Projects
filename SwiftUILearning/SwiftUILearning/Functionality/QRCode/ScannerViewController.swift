...
class ScannerViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    private func configureCamera() {
        DispatchQueue.main.async {
            ...
            if session.canAddOutput(output) {
                ...
                output.metadataObjectTypes = [.qr, .ean13, .ean8, .code128, .aztec, .pdf417, .dataMatrix]
            }
            ...
            // ✅ Restrict scanning only inside the visible square box
            let scanRectWidth: CGFloat = 250
            let scanRectHeight: CGFloat = 250
            let scanRect = CGRect(
    ...
    private func startScanAnimation() {
        ...
        let startY = (view.bounds.height - 250) / 2
        let endY = startY + 250 - 2
