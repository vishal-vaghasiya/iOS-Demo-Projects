...
let service = AnimalIdentifierService()

service.identifyAnimal(image: UIImage(named: "tiger_sample")!) { result in
    if let result = result {
        print("🐾 Label: \(result.label)")
        print("📊 Confidence: \(Int(result.confidence * 100))%")
        print("ℹ️ Short: \(result.shortDescription ?? "-")")
        print("📖 Full: \(result.fullDescription ?? "-")")
        print("🔬 Scientific: \(result.scientificName ?? "-")")
        print("🌐 Wiki Image: \(result.wikiImageURL ?? "-")")
    }
}
